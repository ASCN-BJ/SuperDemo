...
public class CharacterParser {
	private static int[] pinyinValue = new int[] { -20319, -20317, -20304,
			...
			-10254 };
	public static String[] pinyinStr = new String[] { "a", "ai", "an", "ang",
	 ...
	 * @return
	 */
	public String parse(String singleChars) {
		String result = null;
		int ascii = getChsAscii(singleChars);
	 ...
	 * @return
	 */
	public String parse(String wordGroup, boolean unmeaning) {
		String key, value;
		buffer = new StringBuffer();
		for (int i = 0; i < wordGroup.length(); i++) {
			key = wordGroup.substring(i, i + 1);
			if (key.getBytes().length >= 2) {
				value = (String) parse(key);
				if (value == null) {
					value = "#";
				}
			} else {
